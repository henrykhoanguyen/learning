...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // Pull values from a flow to an interim sink to print
        Sink<Transfer, CompletionStage<Done>> transferLogger = Sink.foreach(trasfer -> {
            System.out.println("Transfer from " + trasfer.getFrom().getAccountNumber() +
                    " to " + trasfer.getTo().getAccountNumber() + " of " + trasfer.getTo().getAmount());
        });

        Flow<Transaction, Transaction, NotUsed> applyTransactionsToAccounts = Flow.of(Transaction.class)
                .map( trans -> {
