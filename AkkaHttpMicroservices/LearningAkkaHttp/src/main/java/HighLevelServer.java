...
public class HighLevelServer {
    ...
    Function<HttpRequest, CompletionStage<HttpResponse>> updateCustomer = (request) -> {
        ...
        // check for customer for not null
    ...
    Function<HttpRequest, CompletionStage<HttpResponse>> getAllCustomer = (request) -> {
        CompletableFuture<HttpResponse> response = new CompletableFuture<>();

        CompletionStage<List<Customer>> customerFuture = AskPattern.ask(actorSystem, me -> new CustomerBehavior.GetCustomersCommand(me),
    ...
    Function<HttpRequest, CompletionStage<HttpResponse>> getSingleCustomer = (request) -> {
        ...
        int id = Integer.getInteger(request.getUri().query().get("id").get());
            ...
            if (throwable == null){
                try {
                    if (customer.getId() == 0){
                        ...
                        response.complete(HttpResponse.create().withStatus(StatusCodes.OK).withEntity(json));
                        request.discardEntityBytes(actorSystem);
                    }
...
//        Route getAllCustomerRoute = get( () -> handle(getAllCustomer));
...
//        Route allRoute = pathPrefix("api", () -> customerRoute);
//        return allRoute;
    }


    public void run(){
        CompletionStage<ServerBinding> server =
